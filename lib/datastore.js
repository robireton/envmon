...
export default {
  ...
  co2: (start, stop) => selectConcentration.all({ start: start, stop: stop }).map(row => [row.timestamp, row.ppm]),
  fahrenheit: (start, stop) => selectTemperature.all({ start: start, stop: stop }).map(row => [row.timestamp, temperature.fahrenheit(row.k16)])
}
