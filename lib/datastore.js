...
exports.co2 = (start, stop) => selectConcentration.all({ start: start, stop: stop }).filter((row, i, rs) => (i > 0 && i < (rs.length - 1)) ? (row.ppm !== rs[i - 1].ppm || row.ppm !== rs[i + 1].ppm) : true).map(row => [row.timestamp, row.ppm])

exports.fahrenheit = (start, stop) => selectTemperature.all({ start: start, stop: stop }).filter((row, i, rs) => (i > 0 && i < (rs.length - 1)) ? (row.k16 !== rs[i - 1].k16 || row.k16 !== rs[i + 1].k16) : true).map(row => [row.timestamp, temperature.fahrenheit(row.k16)])
